...
<script>
export default {
  // when we load the page, get all the photos.
  mounted() {
    this.getPhotos();
  },

  // when we enter the photo album, we want to see photos, fetch all photos
  data: () => ({
    photos: [],
    albumName: "",
  }),
  ...
  methods: {
    ...
    // after we load them all into state,
    async getPhotos() {
      const album = await this.$store.dispatch("albumInfo/getAlbum", this.id);
      // tested these out in localhost:20020
